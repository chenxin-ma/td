...
class Stgy_MA(Stgy):
    ...
    def __init__(self, sh, short=8, med=21, far=55, ema=False, priceToUse='close', init=10000):
    ...
    def buyCheck(self, day):
        ...
        if (day[self.priceToUse] > day[self.columnMed] and day[self.columnMed] > day[self.columnFar]):
            logger.debug('%dSMA %.3f > %dSMA %.3f.' %(self.med, day[self.columnMed], self.far, day[self.columnFar]))
